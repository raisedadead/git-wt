...
wt() {
    ...
    elif [[ "$1" == "add" || "$1" == "new" || "$1" == "create" || "$1" == "a" ]]; then
...
function wt --wraps='command wt' --description 'git worktree manager'
    if test (count $argv) -gt 0 && begin; test "$argv[1]" = "switch" || test "$argv[1]" = "cd" || test "$argv[1]" = "sw"; end
    ...
    else if test (count $argv) -gt 0 && begin; test "$argv[1]" = "add" || test "$argv[1]" = "new" || test "$argv[1]" = "create" || test "$argv[1]" = "a"; end
